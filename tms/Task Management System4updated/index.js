...
function subtaskbtn(td) {
  ...
  document.getElementById("subtaskid").value =
    row.cells[0].innerHTML + "." + `${parentid[pid.toString()].length}`;
}
...
function subinsertNewRecord(data) {
  var subtable = document
    .getElementById("subfulltable")
...
function subsearchinput() {
    const searchInput = document.getElementById("search");
    const dataTable = document.getElementById("subfulltable");
    const rows = dataTable.getElementsByTagName("tr");
